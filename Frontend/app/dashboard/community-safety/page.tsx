...
const unwrapArray = <T,>(raw: T[] | ApiEnvelope<T[]> | null | undefined): T[] => {
  if (Array.isArray(raw)) return raw;
  if (Array.isArray(raw?.data)) return raw.data;
  return [];
};

const unwrapItem = <T,>(raw: T | ApiEnvelope<T> | null | undefined): T | null => {
  if (!raw) return null;
  return "data" in raw ? raw.data : raw;
};

const normalizeReport = (report: any): ProductReport => ({
  ...report,
  product_name: report.product_name || "Unknown product",
  store_name: report.store_name || report.purchase_location || null,
  store_location: report.store_location || null,
  images: Array.isArray(report.images)
    ? report.images.map((image: any) => ({
        ...image,
        created_at: image.created_at || image.uploaded_at,
      }))
    : [],
});
...
const normalizeCluster = (cluster: any, productNames: ProductNameMap = {}): IssueCluster => ({
  ...
  issue_type: cluster.issue_type || cluster.primary_issue_type,
  total_reports: cluster.total_reports ?? cluster.affected_reports_count ?? 0,
  risk_score: calculateClusterRisk(cluster),
});

const normalizeAlert = (alert: any): SafetyAlert => ({
  ...alert,
  message: alert.message || alert.description || alert.title || "Safety alert generated.",
});

const normalizeTimeline = (event: any): CaseTimeline => ({
  ...event,
  description: event.description || event.event_description || "",
  created_by: event.created_by || event.performed_by || "SYSTEM",
});

const normalizeCase = (caseObj: any): InvestigationCase => ({
  ...caseObj,
  case_code: caseObj.case_code || caseObj.case_number,
  resolution_summary: caseObj.resolution_summary || caseObj.resolution || null,
  resolved_at: caseObj.resolved_at || caseObj.closed_at || null,
});
...
export default function CommunitySafetyPage() {
  ...
  const fetchInitialData = async () => {
    ...
    try {
      if (activeTab === "overview") {
        const [reportsRaw, alertsRaw, casesRaw, clustersRaw] = await Promise.all([
          apiFetch<any>("/community/reports?limit=50"),
          apiFetch<any>("/community/alerts?limit=50"),
          apiFetch<any>("/community/cases?limit=50"),
          apiFetch<any>("/community/clusters?limit=50"),
        ]);
        const reportsList = unwrapArray<any>(reportsRaw).map(normalizeReport);
        const productNames = buildProductNameMap(reportsList);
        const alertsList = unwrapArray<any>(alertsRaw).map(normalizeAlert);
        const casesList = unwrapArray<any>(casesRaw).map(normalizeCase);
        const clustersList = unwrapArray<any>(clustersRaw).map((cluster) => normalizeCluster(cluster, productNames));
        ...
        // Compute local stats
        const activeAlertCount = alertsList.filter(a => a.status === "ACTIVE").length;
        const openCaseCount = casesList.filter(c => c.status !== "CLOSED" && c.status !== "RESOLVED").length;
        ...
        const data = unwrapArray<any>(await apiFetch<any>("/community/reports?limit=100")).map(normalizeReport);
        setReports(data);
      } else if (activeTab === "clusters") {
        const [clustersRaw, reportsRaw] = await Promise.all([
          apiFetch<any>("/community/clusters?limit=100"),
          apiFetch<any>("/community/reports?limit=100"),
        ]);
        const productNames = buildProductNameMap(unwrapArray<any>(reportsRaw).map(normalizeReport));
        const data = unwrapArray<any>(clustersRaw).map((cluster) => normalizeCluster(cluster, productNames));
        setClusters(data);
      } else if (activeTab === "alerts") {
        const data = unwrapArray<any>(await apiFetch<any>("/community/alerts?limit=100")).map(normalizeAlert);
        setAlerts(data);
      } else if (activeTab === "cases") {
        const data = unwrapArray<any>(await apiFetch<any>("/community/cases?limit=100")).map(normalizeCase);
        setCases(data);
      }
    } catch (err: any) {
  ...
  const handleViewReport = async (report: ProductReport) => {
    ...
    try {
      const res = unwrapItem<CredibilityResponse>(await apiFetch<any>(`/community/reports/${report.id}/credibility`));
      setSelectedReportCredibility(res);
    } catch (err: any) {
      console.error(err);
    }
  };
  ...
  const handleRecalculateCredibility = async (reportId: string) => {
    ...
    try {
      const res = unwrapItem<CredibilityResponse>(await apiFetch<any>(`/community/reports/${reportId}/recalculate`, {
        method: "POST"
      }));
      setSelectedReportCredibility(res);
    } catch (err: any) {
  ...
  const handleViewCluster = async (cluster: IssueCluster) => {
    ...
    try {
      const res = unwrapArray<any>(await apiFetch<any>(`/community/clusters/${cluster.id}/reports`)).map(normalizeReport);
      setSelectedClusterReports(res);
    } catch (err: any) {
      console.error(err);
    }
  };
  ...
  const handleAcknowledgeAlert = async (alertId: string) => {
    ...
    try {
      ...
      fetchInitialData();
      if (selectedAlert) {
        const updated = unwrapItem<any>(await apiFetch<any>(`/community/alerts/${alertId}`));
        setSelectedAlert(updated ? normalizeAlert(updated) : null);
      }
    } catch (err: any) {
  ...
  const fetchCaseFolderDetails = async (caseId: string) => {
    try {
      const caseDetails = unwrapItem<any>(await apiFetch<any>(`/community/cases/${caseId}`));
      if (!caseDetails) return;

      setSelectedCase(normalizeCase(caseDetails));
      setCaseNotes(unwrapArray<CaseNote>(caseDetails.notes));
      setCaseEvidence(unwrapArray<CaseEvidence>(caseDetails.evidence));
      setCaseTimeline(unwrapArray<any>(caseDetails.timeline).map(normalizeTimeline));
    } catch (err) {
      console.error(err);
    }
  };
  ...
  const handleAssignCase = async () => {
    ...
    setActionLoading(true);
    try {
      const updated = unwrapItem<any>(await apiFetch<any>(
        `/community/cases/${selectedCase.id}/assign?officer=${encodeURIComponent(assignee)}`,
        { method: "PATCH" }
      ));
      setSelectedCase(updated ? normalizeCase(updated) : null);
  ...
  return (
    <div className="space-y-8 p-8">
      ...
      <div className="flex border-b border-gray-200">
        ...
        >
          Customer Reports ({safeReports.length})
        </button>
        <button
        ...
        >
          Issue Clusters ({safeClusters.length})
        </button>
        <button
        ...
        >
          Safety Alerts ({safeAlerts.filter(a => a.status === "ACTIVE").length})
        </button>
        <button
        ...
        >
          Investigations & Cases ({safeCases.length})
        </button>
      </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden flex flex-col">
                  ...
                  <div className="p-5 divide-y divide-gray-100 overflow-y-auto max-h-[360px]">
                    {safeAlerts.filter(a => a.status === "ACTIVE").length > 0 ? (
                      safeAlerts.filter(a => a.status === "ACTIVE").map(alert => (
                ...
                <div className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden flex flex-col">
                  ...
                  <div className="p-5 divide-y divide-gray-100 overflow-y-auto max-h-[360px]">
                    {safeCases.filter(c => c.status !== "CLOSED" && c.status !== "RESOLVED").length > 0 ? (
                      safeCases.filter(c => c.status !== "CLOSED" && c.status !== "RESOLVED").map(c => (
