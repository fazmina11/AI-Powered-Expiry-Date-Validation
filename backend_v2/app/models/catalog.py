...
class ProductIdentifier(Base):
    ...
    product_id = Column(UUID(as_uuid=True), ForeignKey("products.products.id", ondelete="CASCADE"), nullable=False, index=True)
...
class ProductNutrition(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4, index=True)
    product_id = Column(UUID(as_uuid=True), ForeignKey("products.products.id", ondelete="CASCADE"), nullable=False, unique=True, index=True)
